...
func (m *MTProto) sendPacket(msg TL, resp chan TL) error {
	...
	if m.encrypted {
		...
		z.Bytes(obj)
		messageSize := len(obj)
		additionalSize := (32 + messageSize) % 16
		if additionalSize != 0 {
		...
		// glog.V(3).Infof("len(data): %d before additionalSize: %d", len(z.Buf), additionalSize)
		if additionalSize != 0 {
			listRand := make([]byte, additionalSize)
			rand.Read(listRand)

			z.Buf = append(z.Buf, listRand...)
		}
		// Note - Important
		rawKey := make([]byte, len(m.authKey))
		copy(rawKey, m.authKey)
		s256 := sha256.Sum256(append(rawKey[88:120], z.Buf...))
		msgKey:=s256[8:24]
		aesKey, aesIV := generateAES(msgKey, m.authKey, false)

		y := make([]byte, len(z.Buf))
		copy(y, z.Buf)
...
func (m *MTProto) read(stop <-chan struct{}) (interface{}, error) {
	...
	if binary.LittleEndian.Uint64(authKeyHash) == 0 {
		...
		//}
		msgID, seqNo, data, err = Decrypt(m.authKey, dbuf)

		//fmt.Printf("Data receive after decrypt: %v\n", data)
		if err != nil {
			fmt.Println(err)
		}
		...
		//fmt.Printf("Before parse object m.Off+4 : %d m.Size : %d\n",dbuf.Off+4,dbuf.Size)
		fmt.Println("1")
		if data == nil {
			fmt.Println("4")
			data = dbuf.Vector()
